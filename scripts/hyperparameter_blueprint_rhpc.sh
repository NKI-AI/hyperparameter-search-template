...
# Hyperparameter config -- multirun example:
...
source activate "graph_cytof"

# Slurm setup, some parameters might depend on the names specific to your cluster
SLURM_PARAMETERS="hydra.launcher.gpus_per_node=$NUM_GPUS_PER_NODE hydra.launcher.nodes=$NUM_NODES hydra.launcher.cpus_per_task=$NUM_CPUS_PER_NODE hydra.launcher.mem_gb=$MEM_GB_PER_NODE"
...
# start mlflow server -- here we could also specify external storages (such as AWS S3)
